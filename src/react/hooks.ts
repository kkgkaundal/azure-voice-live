...
import { createClientSync } from '../index';
...
export function useVoiceLive(options: UseVoiceLiveOptions): UseVoiceLiveReturn {
  ...
  const initializeClient = useCallback(async () => {
    ...
    try {
      // Use synchronous version for React hooks to avoid async issues
      clientRef.current = createClientSync(options.config);
...
// Additional hook for simpler usage without auto-initialization
export function useVoiceLiveClient(config: VoiceLiveConfig): {
  ...
  const createClientInstance = useCallback(() => {
    try {
      setError(null);
      const newClient = createClientSync(config);
      setClient(newClient);
      return newClient;
    } catch (err) {
      setError(err as Error);
